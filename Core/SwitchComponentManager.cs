...
public sealed class SwitchComponentManager
{
    ...
    {
        _paths = paths; _logger = logger; _releases = new GitHubReleaseClient(logger); _stateFile = Path.Combine(paths.DataDirectory, "components.json"); _state = new JsonStore<ComponentManagerState>(_stateFile).LoadOrCreate();
    }
    ...
    public async Task<ComponentStatus> InstallOrUpdateAsync(SwitchComponent component, string targetRoot, IProgress<DownloadProgress>? progress, CancellationToken ct = default)
    {
        ...
        if (component is SwitchComponent.Linux or SwitchComponent.Tools) throw new InvalidOperationException($"{definition.Name} is not yet an automatic release component.");
        if (!Directory.Exists(targetRoot)) throw new DirectoryNotFoundException(targetRoot);
        var release = await GetReleaseAsync(definition, ct);
        ...
        try
        {
            ...
            {
                var stage = Path.Combine(componentCache, "staging", Guid.NewGuid().ToString("N")); Directory.CreateDirectory(stage);
                try { ExtractSafe(downloadPath, stage); var sourceRoot = FindPayloadRoot(stage, definition.DetectionPath); MergeDirectory(sourceRoot, targetRoot); if (!File.Exists(Path.Combine(targetRoot, definition.DetectionPath))) throw new InvalidDataException($"The downloaded archive did not produce the expected {definition.DetectionPath}."); }
                finally { TryDeleteDirectory(stage); }
            }
            _state.LastTargetRoot = targetRoot; _state.LastKnownVersions[component.ToString()] = release.TagName; _state.UpdatedAt = DateTimeOffset.UtcNow; new JsonStore<ComponentManagerState>(_stateFile).Save(_state); transaction.Commit();
        }
        catch
        {
    ...
    {
        var a = ExtractNumbers(installed); var b = ExtractNumbers(available); for (var i = 0; i < Math.Max(a.Count, b.Count); i++) { var av = i < a.Count ? a[i] : 0; var bv = i < b.Count ? b[i] : 0; if (av != bv) return av.CompareTo(bv); } return string.Equals(installed, available, StringComparison.OrdinalIgnoreCase) ? 0 : -1;
    }
    ...
    private static void ExtractSafe(string archivePath, string destination)
    {
        ...
        foreach (var entry in archive.Entries.Where(e => !e.IsDirectory)) { var key = entry.Key; if (string.IsNullOrWhiteSpace(key)) throw new InvalidDataException("Archive contains an entry without a path."); var relative = key.Replace('/', Path.DirectorySeparatorChar).Replace('\\', Path.DirectorySeparatorChar); var full = Path.GetFullPath(Path.Combine(destination, relative)); if (!full.StartsWith(root, StringComparison.OrdinalIgnoreCase)) throw new InvalidDataException("Archive contains an unsafe path."); Directory.CreateDirectory(Path.GetDirectoryName(full)!); entry.WriteToFile(full, new ExtractionOptions { ExtractFullPath = false, Overwrite = true }); }
    }
    ...
    {
        var candidates = definition.Id switch { SwitchComponent.Hekate => new[] { "bootloader" }, SwitchComponent.Atmosphere => new[] { "atmosphere" }, SwitchComponent.Dbi => new[] { Path.Combine("switch", "DBI") }, _ => Array.Empty<string>() };
        if (candidates.Length == 0) return ""; var backupRoot = Path.Combine(targetRoot, "_mewswitch-backups", $"{definition.Id}-{DateTime.UtcNow:yyyyMMdd-HHmmss}-{SanitizeFileName(version)}");
        foreach (var relative in candidates) { var source = Path.Combine(targetRoot, relative); if (!Directory.Exists(source) && !File.Exists(source)) continue; var destination = Path.Combine(backupRoot, relative); if (Directory.Exists(source)) CopyDirectory(source, destination); else { Directory.CreateDirectory(Path.GetDirectoryName(destination)!); File.Copy(source, destination, true); } }
        return backupRoot;
    }
    private static void RestoreBackup(string targetRoot, string backupRoot)
    {
        if (!Directory.Exists(backupRoot)) return; foreach (var file in Directory.GetFiles(backupRoot, "*", SearchOption.AllDirectories)) { var relative = Path.GetRelativePath(backupRoot, file); var destination = Path.Combine(targetRoot, relative); Directory.CreateDirectory(Path.GetDirectoryName(destination)!); File.Copy(file, destination, true); }
    }
    ...
    {
        Directory.CreateDirectory(destination); foreach (var file in Directory.GetFiles(source, "*", SearchOption.AllDirectories)) { var target = Path.Combine(destination, Path.GetRelativePath(source, file)); Directory.CreateDirectory(Path.GetDirectoryName(target)!); File.Copy(file, target, true); }
    }
    private static string SanitizeFileName(string value) { foreach (var c in Path.GetInvalidFileNameChars()) value = value.Replace(c, '_'); return string.IsNullOrWhiteSpace(value) ? "component.bin" : value; }
