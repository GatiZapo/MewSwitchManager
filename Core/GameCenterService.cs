...
/// <summary>
/// Handles user-provided content without becoming a game-download catalogue.
/// The service verifies local content, stages it atomically and keeps the source
/// untouched. Installer-specific deployment remains an adapter concern.
/// </summary>
public sealed class GameCenterService(AppLogger logger)
{
    private static readonly HashSet<string> SupportedExtensions = new(StringComparer.OrdinalIgnoreCase)
    {
        ".nsp", ".nsz", ".xci", ".xcz", ".nro", ".nca", ".zip"
    };
    ...
    public async Task<GameContentInfo> InspectAsync(string sourcePath, CancellationToken ct = default)
    {
        ...
        if (!SupportedExtensions.Contains(info.Extension))
            throw new InvalidDataException($"Unsupported Game Center file type: {info.Extension}");
    ...
    public async Task<GameStageResult> StageAsync(GameContentInfo content, string destinationRoot, CancellationToken ct = default)
    {
        ...
        if (sourceInfo.Length != content.SizeBytes)
            throw new IOException("The source changed after indexing. Re-run preflight before staging.");

        var incoming = Path.Combine(destinationRoot, "MewNX", "Incoming");
        Directory.CreateDirectory(incoming);
        var safeName = SanitizeFileName(content.DisplayName);
        var finalPath = Path.Combine(incoming, safeName);
        var partPath = finalPath + ".part";

        var available = new DriveInfo(Path.GetPathRoot(Path.GetFullPath(destinationRoot))!).AvailableFreeSpace;
        if (available < content.SizeBytes)
            throw new IOException($"Not enough free space for staging. Required: {content.SizeBytes:N0} bytes; available: {available:N0} bytes.");
        ...
        try
        {
            ...
            if (!string.Equals(stagedHash, content.Sha256, StringComparison.OrdinalIgnoreCase))
                throw new InvalidDataException("SHA-256 verification failed after staging. The staged file was discarded.");
