...
public sealed class DependencyManager
{
    ...
    {
        using var stream = File.OpenRead(manifestPath); using var document = JsonDocument.Parse(stream);
        if (!document.RootElement.TryGetProperty("components", out var components) || components.ValueKind != JsonValueKind.Array) throw new InvalidDataException("Dependency manifest has no components array.");
        ...
        foreach (var item in components.EnumerateArray())
        {
            ...
            if (string.IsNullOrWhiteSpace(id) || string.IsNullOrWhiteSpace(name)) throw new InvalidDataException("Dependency manifest contains a component without id or name.");
            var deps = item.TryGetProperty("dependencies", out var d) && d.ValueKind == JsonValueKind.Array ? d.EnumerateArray().Select(x => x.GetString()).Where(x => !string.IsNullOrWhiteSpace(x)).Cast<string>().ToArray() : Array.Empty<string>();
            VersionConstraint? constraint = null; if (item.TryGetProperty("versionConstraint", out var c) && c.ValueKind == JsonValueKind.Object) constraint = JsonSerializer.Deserialize<VersionConstraint>(c.GetRawText());
    ...
    public DependencyPlan BuildPlan(IEnumerable<ComponentManifestEntry> manifest, IEnumerable<string> requested, IReadOnlyDictionary<string, string> installedVersions)
    {
        ...
        bool Visit(string id)
        {
            ...
            if (!entries.TryGetValue(id, out var entry)) { missing.Add(id); blocked.Add(id); return false; }
            if (!visiting.Add(id)) { cycles.Add(id); blocked.Add(id); return false; }
            var healthy = true; foreach (var dependency in entry.Dependencies) if (!Visit(dependency)) healthy = false;
            visiting.Remove(id); visited.Add(id);
            if (installedVersions.TryGetValue(id, out var installedVersion) && entry.Constraint is not null && !string.Equals(installedVersion, "installed", StringComparison.OrdinalIgnoreCase) && !entry.Constraint.Allows(installedVersion)) { incompatible.Add(id); healthy = false; }
            if (!healthy) { blocked.Add(id); return false; }
            if (!installedVersions.ContainsKey(id)) order.Add(id); return true;
        }
        foreach (var id in requested.Where(x => !string.IsNullOrWhiteSpace(x))) Visit(id);
        return new(order.Distinct(StringComparer.OrdinalIgnoreCase).ToArray(), missing.ToArray(), cycles.ToArray(), incompatible.ToArray());
    }
}
