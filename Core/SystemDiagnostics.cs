...
public sealed class SystemDiagnostics
{
    ...
    public Task<DiagnosticReport> RunAsync(InstallationEngine engine, CancellationToken ct = default)
    {
        ...
        var freeBytes = 0L;
        try { var root = Path.GetPathRoot(_paths.CacheDirectory); if (!string.IsNullOrWhiteSpace(root)) freeBytes = new DriveInfo(root).AvailableFreeSpace; } catch (Exception ex) { _logger.Warn($"Diagnostics disk-space probe failed: {ex.Message}"); }
        const long recommendedFree = 10L * 1024 * 1024 * 1024;
        checks.Add(freeBytes >= recommendedFree ? new("space", "Working storage", DiagnosticSeverity.Pass, $"{freeBytes / 1024d / 1024d / 1024d:F1} GiB available; recommended minimum is 10 GiB.") : new("space", "Working storage", freeBytes > 0 ? DiagnosticSeverity.Warning : DiagnosticSeverity.Fail, freeBytes > 0 ? $"Only {freeBytes / 1024d / 1024d / 1024d:F1} GiB is available; downloads/extraction may fail." : "Could not determine available working storage."));
        ...
        checks.Add(catalog.IsValid ? new("catalog", "Component catalog", DiagnosticSeverity.Pass, $"Schema {catalog.SchemaVersion} • {catalog.Components.Count} components • dependency references valid.") : new("catalog", "Component catalog", DiagnosticSeverity.Fail, string.Join(" | ", catalog.Errors)));
        ...
        if (sd is null) checks.Add(new("sd", "Switch SD", DiagnosticSeverity.Warning, "No Switch-like removable storage is currently mounted."));
        else
        {
        ...
        var report = new DiagnosticReport(DateTimeOffset.UtcNow, checks); _logger.Info($"Diagnostics completed: {checks.Count(x => x.Severity == DiagnosticSeverity.Pass)} pass, {checks.Count(x => x.Severity == DiagnosticSeverity.Warning)} warning, {checks.Count(x => x.Severity == DiagnosticSeverity.Fail)} fail."); return Task.FromResult(report);
    }
