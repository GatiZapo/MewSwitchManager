...
public sealed class RepairService
{
    ...
    public RepairService(AppPaths paths, AppConfig config, AppLogger logger) { _paths = paths; _config = config; _logger = logger; }
    ...
    private static RepairAction EnsureDirectory(string path, string id)
    {
        ...
        Directory.CreateDirectory(path); return new(id, true, $"Created missing directory: {path}");
    }
    ...
    private RepairAction RepairStateFile(string path, CancellationToken ct)
    {
        ...
        try
        {
            var store = new JsonStore<AppState>(path); var state = store.LoadOrCreate(); state.EnsureStages(); state.ReconcilePersistedProgress();
            var existed = File.Exists(path); store.Save(state);
            return new("state", !existed, existed ? "Persisted workflow state validated and normalized." : "Created missing workflow state.");
        }
        catch (Exception ex) { _logger.Warn($"Workflow state repair failed: {ex.Message}"); return new("state", false, "Workflow state could not be repaired automatically; the original file was left untouched."); }
    }
    ...
    private RepairAction RepairComponentState(string path, CancellationToken ct)
    {
        ...
        try
        {
            var store = new JsonStore<ComponentManagerState>(path); var state = store.LoadOrCreate(); var existed = File.Exists(path); store.Save(state);
            return new("components", !existed, existed ? "Component state validated and normalized." : "Created missing component state.");
        }
        catch (Exception ex) { _logger.Warn($"Component state repair failed: {ex.Message}"); return new("components", false, "Component state could not be repaired automatically."); }
    }
    ...
    private RepairAction RepairLinuxImageCache(CancellationToken ct)
    {
        ...
        try
        {
            var imagePath = new LinuxImageService(new HttpClient(), _logger, _config).FinalPath(_paths.CacheDirectory);
            ...
            File.Delete(imagePath); _logger.Warn($"Removed zero-byte Linux image cache: {imagePath}");
            return new("linux-image", true, "Removed a zero-byte Linux image so the next download can resume cleanly.");
        }
        catch (Exception ex) { _logger.Warn($"Linux image repair failed: {ex.Message}"); return new("linux-image", false, "Linux image cache could not be repaired automatically."); }
    }
    ...
    private RepairAction CleanupStaleTransactions(CancellationToken ct)
    {
        ...
        {
            ct.ThrowIfCancellationRequested();
            try { if (Directory.GetLastWriteTimeUtc(directory) >= DateTime.UtcNow.AddDays(-1)) continue; Directory.Delete(directory, true); removed++; }
            catch (Exception ex) { _logger.Warn($"Could not remove stale transaction {directory}: {ex.Message}"); }
        }
        try { if (!Directory.EnumerateFileSystemEntries(transactionRoot).Any()) Directory.Delete(transactionRoot, false); } catch { }
