...
public sealed class EmulationInstaller
{
    ...
    {
        ValidateTarget(targetRoot); EnsureFreeSpace(targetRoot);
    ...
    public async Task<EmulationInstallResult> InstallOrUpdateAsync(EmulationPackageDefinition definition, string targetRoot, IProgress<DownloadProgress>? progress = null, CancellationToken ct = default)
    {
        ...
        var cache = Path.Combine(_paths.CacheDirectory, "emulation", definition.Id); Directory.CreateDirectory(cache);
        ...
        try
        {
            if (definition.InstallMode == EmulationInstallMode.DirectFile)
            {
                ...
                Directory.CreateDirectory(Path.GetDirectoryName(destination)!); File.Copy(downloadPath, destination, true);
            ...
            else
            {
                ...
                try
                {
                    ExtractSafe(downloadPath, stage); ValidateRetroArchBundle(stage);
                    MergePreservingUserData(stage, targetRoot, definition.PreserveRelativePaths ?? Array.Empty<string>());
                    if (!File.Exists(Path.Combine(targetRoot, "switch", "retroarch.nro")) || !Directory.Exists(Path.Combine(targetRoot, "retroarch", "cores"))) throw new InvalidDataException("RetroArch bundle is incomplete: switch/retroarch.nro or retroarch/cores is missing.");
                }
    ...
    private static void ValidateTarget(string targetRoot) { if (!Directory.Exists(targetRoot)) throw new DirectoryNotFoundException(targetRoot); }

    private static void EnsureFreeSpace(string targetRoot)
    {
        var root = Path.GetPathRoot(Path.GetFullPath(targetRoot)); if (string.IsNullOrWhiteSpace(root)) return;
        var drive = new DriveInfo(root);
        if (drive.AvailableFreeSpace < RecommendedFreeBytes) throw new IOException($"The Switch SD card needs at least 4 GB of free space for the full emulation stack. Available: {drive.AvailableFreeSpace / 1024d / 1024d / 1024d:F1} GB.");
    }
    ...
    {
        var backupRoot = Path.Combine(targetRoot, "_mewswitch-backups", "emulation", $"{DateTime.UtcNow:yyyyMMdd-HHmmss}-{Sanitize(definition.Id)}"); var copied = false;
        if (definition.InstallMode == EmulationInstallMode.DirectFile)
        {
            var source = Path.Combine(targetRoot, definition.Destination);
            if (File.Exists(source)) { var destination = Path.Combine(backupRoot, definition.Destination); Directory.CreateDirectory(Path.GetDirectoryName(destination)!); File.Copy(source, destination, true); copied = true; }
        }
        else
        {
            ...
            if (File.Exists(nro)) { var destination = Path.Combine(backupRoot, "switch", "retroarch.nro"); Directory.CreateDirectory(Path.GetDirectoryName(destination)!); File.Copy(nro, destination, true); copied = true; }
        }
    ...
    private static void RestoreBackup(string targetRoot, string backupRoot, EmulationPackageDefinition definition)
    {
        if (definition.InstallMode == EmulationInstallMode.ArchiveToRoot)
        {
            ...
            var backupRetro = Path.Combine(backupRoot, "retroarch");
            ...
            var backupNro = Path.Combine(backupRoot, "switch", "retroarch.nro");
            if (File.Exists(targetNro)) File.Delete(targetNro);
            if (File.Exists(backupNro)) { Directory.CreateDirectory(Path.GetDirectoryName(targetNro)!); File.Copy(backupNro, targetNro, true); }
            return;
        }
    ...
    {
        var root = Path.GetFullPath(destination) + Path.DirectorySeparatorChar; using var archive = ArchiveFactory.OpenArchive(archivePath);
        foreach (var entry in archive.Entries.Where(e => !e.IsDirectory))
        {
            var relative = entry.Key.Replace('/', Path.DirectorySeparatorChar).Replace('\\', Path.DirectorySeparatorChar); var full = Path.GetFullPath(Path.Combine(destination, relative));
            ...
            Directory.CreateDirectory(Path.GetDirectoryName(full)!); entry.WriteToFile(full, new ExtractionOptions { ExtractFullPath = false, Overwrite = true });
        }
    }

    private static void ValidateRetroArchBundle(string stage)
    {
        var nro = Directory.GetFiles(stage, "retroarch.nro", SearchOption.AllDirectories).FirstOrDefault(); var cores = Directory.GetDirectories(stage, "cores", SearchOption.AllDirectories).FirstOrDefault();
    ...
    {
        foreach (var directory in Directory.GetDirectories(source, "*", SearchOption.AllDirectories)) { var relative = Path.GetRelativePath(source, directory); if (!IsPreserved(relative, preserved)) Directory.CreateDirectory(Path.Combine(destination, relative)); }
        foreach (var file in Directory.GetFiles(source, "*", SearchOption.AllDirectories)) { var relative = Path.GetRelativePath(source, file); if (IsPreserved(relative, preserved)) continue; var target = Path.Combine(destination, relative); Directory.CreateDirectory(Path.GetDirectoryName(target)!); File.Copy(file, target, true); }
    }

    private static bool IsPreserved(string relative, IReadOnlyList<string> preserved)
    {
        var normalized = relative.Replace('\\', '/'); return preserved.Any(p => normalized.Equals(p.Replace('\\', '/'), StringComparison.OrdinalIgnoreCase) || normalized.StartsWith(p.Replace('\\', '/') + "/", StringComparison.OrdinalIgnoreCase));
    }
    ...
    private static async Task VerifyDownloadedFileAsync(string path, string? digest, CancellationToken ct)
    {
        ...
        await using var stream = File.OpenRead(path); using var sha = SHA256.Create(); var actual = Convert.ToHexString(await sha.ComputeHashAsync(stream, ct)); var expected = digest[7..].Trim().ToUpperInvariant();
    ...
    private static string Sanitize(string value) { foreach (var c in Path.GetInvalidFileNameChars()) value = value.Replace(c, '_'); return value.Replace('/', '_').Replace('\\', '_'); }
