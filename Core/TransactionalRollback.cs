...
/// A transaction snapshots individual files or complete component directories.
/// Rollback restores overwritten files, removes newly created files/directories,
/// and verifies the resulting state before reporting success.
/// </summary>
public sealed class TransactionalRollback : IDisposable
{
    ...
    public RollbackResult Rollback()
    {
        ...
        foreach (var capturedDirectory in _capturedDirectories)
        {
            try
            {
                ...
                if (!Directory.Exists(capturedDirectory.Key))
                {
                    Directory.CreateDirectory(capturedDirectory.Key);
                    continue;
                }
    ...
    private RollbackResult VerifyRestoredState()
    {
        ...
        foreach (var pair in _originals)
        {
            if (pair.Value is null)
            {
                if (File.Exists(pair.Key)) errors.Add($"New file still exists after rollback: {pair.Key}");
                continue;
            }
            if (!File.Exists(pair.Key)) { errors.Add($"Original file is missing after rollback: {pair.Key}"); continue; }
            try
            {
                if (!FilesEqual(pair.Value, pair.Key)) errors.Add($"Restored file differs from its snapshot: {pair.Key}");
            }
    ...
    private static bool FilesEqual(string left, string right)
    {
        ...
        var ah = sha.ComputeHash(a);
        sha.Initialize();
        var bh = sha.ComputeHash(b);
