...
/// The implementation deliberately uses the SharpCompress 0.50.x-compatible IArchiveEntry.WriteTo(Stream) API.
/// </summary>
public sealed class LinuxArchiveServiceV2(AppLogger logger)
{
    ...
    public void Cleanup(string workDirectory)
        => TryDeleteDirectory(Path.Combine(workDirectory, "linux-extracted"));
        ...
        CancellationToken ct)
    {
        ...
        foreach (var entry in entries)
        {
            ...
            var entryBase = completedBytes;
            try
            {
                WriteEntryWithRetry(entry, temporaryPath, entryBase, totalBytes, completedEntries, totalEntries, progress, ct);
                MoveWithRetry(temporaryPath, outputPath, ct);
                completedBytes += Math.Max(0L, entry.Size);
                completedEntries++;
                Report(progress,
                    totalBytes > 0 ? completedBytes : completedEntries,
                    totalBytes > 0 ? totalBytes : totalEntries,
        ...
        Report(progress,
            totalBytes > 0 ? totalBytes : totalEntries,
            totalBytes > 0 ? totalBytes : totalEntries,
            "FINALIZING LINUX IMAGE",
            "Extraction complete. Finalizing files...");
        logger.Info("Linux archive extraction completed.");
    }

    private static void WriteEntryWithRetry(
        dynamic entry,
        ...
        CancellationToken ct)
    {
        for (var attempt = 1; attempt <= WriteAttempts; attempt++)
        {
            ...
            try
            {
                using var output = new ProgressFileStream(temporaryPath, written =>
                {
                    ...
                    Report(progress,
                        totalBytes > 0 ? Math.Min(current, totalBytes) : completedEntries,
                        totalBytes > 0 ? totalBytes : totalEntries,
                        "EXTRACTING LINUX IMAGE",
                        $"Extracting {Path.GetFileName((string)entry.Key)}");
                });
        ...
        throw new IOException($"Could not extract archive entry '{entry.Key}'.");
    }
    ...
    private static void Report(IProgress<DownloadProgress>? progress, long bytes, long? total, string phase, string detail)
    ...
    private sealed class ProgressFileStream(string path, Action<long> onProgress) : FileStream(
        path, FileMode.CreateNew, FileAccess.Write, FileShare.ReadWrite | FileShare.Delete, 1024 * 1024, FileOptions.SequentialScan)
    {
        ...
        {
            _written += count;
            if (_clock.ElapsedMilliseconds < 100) return;
            onProgress(_written);
            _clock.Restart();
        }
    }
