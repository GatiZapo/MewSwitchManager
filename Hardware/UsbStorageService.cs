    ...
    MewSwitchManager.Core.SafetyEngine safety)
{
    ...
    private readonly LinuxArchiveService _archive = new(logger);
        ...
        CancellationToken ct = default)
    {
        ...
        try
        {
            rawPath = await _archive.PrepareRawImageAsync(archivePath, workDirectory, progress, ct);
            await FlashAsync(target, rawPath, progress, ct);
        }
        finally
        {
            _archive.Cleanup(workDirectory);
        }
    }
        ...
        CancellationToken ct)
    {
        ...
        if (rawSize <= 0)
            throw new InvalidDataException("The extracted Linux image is empty.");

        if (target.SizeGb * 1_000_000_000d < rawSize)
            throw new InvalidOperationException(
                $"The selected USB is too small. It has about {target.SizeGb:0.0} GB, while the Linux image needs {rawSize / 1_000_000_000d:0.00} GB.");
        ...
        var beforeClean = await _disks.GetDiskAsync(target.Number, ct);
        ...
        var afterPartition = await _disks.GetDiskAsync(target.Number, ct);
    ...
    private static void ValidatePartition(
        (int PartitionNumber, long Size, string? VolumePath, string? DriveLetter)? partition,
        long rawSize)
    {
        ...
        if (partition.Value.Size < rawSize)
            throw new InvalidOperationException(
                $"The USB partition ({partition.Value.Size:N0} bytes) is smaller than the Linux image ({rawSize:N0} bytes).");
    }
    ...
    private async Task<(int PartitionNumber, long Size, string? VolumePath, string? DriveLetter)?> GetFirstPartitionVolumeAsync(
        int diskNumber,
        CancellationToken ct)
    {
...
[pscustomobject]@{{
    ...
    DriveLetter = if ($v -and $v.DriveLetter) {{ [string]$v.DriveLetter }} else {{ '' }}
        ...
        return (
            root.GetProperty("PartitionNumber").GetInt32(),
            root.GetProperty("Size").GetInt64(),
            root.TryGetProperty("VolumePath", out var path) ? path.GetString() : null,
            root.TryGetProperty("DriveLetter", out var drive) ? drive.GetString() : null);
    }
