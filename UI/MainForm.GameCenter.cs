...
public sealed partial class MainForm
{
    ...
    private Control BuildGameCenterSection()
    {
        ...
        var card = CreateCard(); card.Height = 430; card.Padding = new Padding(16);
        ...
        var info = new Label { Dock = DockStyle.Top, Height = 42, Text = "Contenido proporcionado por el usuario • descargas reanudables • ZIP/7Z/TAR/GZ • verificación antes de limpiar temporales.", ForeColor = Theme.Muted, Font = Theme.Mono(7.1f) };
        ...
        StyleButton(_gameAddUrlButton, "ADD URL", Theme.Blue, 105); StyleButton(_gamePickButton, "LOCAL FILE", Theme.Blue, 105); StyleButton(_gameProcessQueueButton, "PROCESS QUEUE", Theme.Pink, 145);
        _gameAddUrlButton.Click += (_, _) => AddGameUrl(); _gamePickButton.Click += async (_, _) => await SelectGameContentAsync(); _gameProcessQueueButton.Click += async (_, _) => await ProcessGameQueueAsync();
        ...
        var queueHost = new Panel { Dock = DockStyle.Fill, Padding = new Padding(0, 4, 0, 0), BackColor = Theme.Surface }; queueHost.Controls.Add(_gameQueueList);
        _gameQueueStatus.Dock = DockStyle.Bottom; _gameQueueStatus.Height = 24; _gameQueueStatus.ForeColor = Theme.Muted; _gameQueueStatus.Font = Theme.Mono(7);
        ...
        RefreshGameQueueList(); return card;
    }
    ...
    private void ConfigureGameQueueList()
    {
        ...
        _gameQueueList.Columns.Add("NAME", 250); _gameQueueList.Columns.Add("STATE", 110); _gameQueueList.Columns.Add("PROGRESS", 120); _gameQueueList.Columns.Add("SOURCE", 420); _gameQueueList.Columns.Add("ERROR", 300); _gameQueue.Changed += RefreshGameQueueList;
    }

    private void RefreshGameQueueList()
    {
        if (InvokeRequired) { BeginInvoke(RefreshGameQueueList); return; }
        _gameQueueList.BeginUpdate(); try
        {
    ...
    private void AddGameUrl()
    {
        ...
        try { var name = Path.GetFileName(uri.AbsolutePath); if (string.IsNullOrWhiteSpace(name)) name = "remote-content"; _gameQueue.AddDirectUrl(name, uri.ToString()); _gameUrl.Clear(); SetStatus("●  GAME DOWNLOAD QUEUED", Theme.Blue); }
    ...
    private async Task ProcessGameQueueAsync()
    {
        ...
        try
        {
            ...
            foreach (var job in pending)
            {
                ...
                {
                    // Safe scope: stage verified user-provided content. No source scraping or unauthorised catalogue is performed.
                ...
                }, new Progress<DownloadProgress>(p => _gameQueueStatus.Text = p.TotalBytes is > 0 ? $"{p.Phase}: {p.BytesReceived * 100d / p.TotalBytes.Value:0.0}% • {p.SpeedBytesPerSecond / 1024d / 1024d:0.0} MiB/s" : $"{p.Phase}: {p.BytesReceived:N0} bytes"), _operationCts.Token);
            }
    ...
    {
        using var dialog = new OpenFileDialog { Title = "MewNX — Select user-provided content", Filter = "Switch/Homebrew content|*.nsp;*.nsz;*.xci;*.xcz;*.nro;*.nca;*.nso;*.kip;*.ovl;*.zip;*.7z;*.tar;*.gz|All files|*.*", CheckFileExists = true, Multiselect = false, RestoreDirectory = true };
        if (dialog.ShowDialog(this) != DialogResult.OK) return;
        try { _gamePickButton.Enabled = false; SetStatus("●  GAME CONTENT VERIFY", Theme.Blue); _gameContent = await _gameCenter.InspectAsync(dialog.FileName); _gameFileLabel.Text = $"READY / VERIFIED  •  {_gameContent.DisplayName}\nSIZE {_gameContent.SizeBytes:N0} bytes  •  TYPE {_gameContent.Extension}\nSHA-256 {_gameContent.Sha256}\nSource remains untouched."; _gameFileLabel.ForeColor = Theme.Green; SetStatus("●  GAME CONTENT READY", Theme.Green); }
