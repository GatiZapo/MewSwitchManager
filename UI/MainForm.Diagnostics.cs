...
public sealed partial class MainForm
{
    ...
    private Control BuildDiagnosticsSection()
    {
        ...
        var title = new Label { Dock = DockStyle.Top, Height = 28, Text = "SYSTEM DIAGNOSTICS", ForeColor = Theme.Text, Font = Theme.UI(12, FontStyle.Bold) };
        _diagnosticsStatus.Dock = DockStyle.Top;
        _diagnosticsStatus.Height = 42;
        _diagnosticsStatus.ForeColor = Theme.Muted;
        _diagnosticsStatus.Font = Theme.Mono(7.5f);
        _diagnosticsStatus.Text = "No diagnostic scan run yet.";

        StyleButton(_diagnosticsButton, "RUN DIAGNOSTICS", Theme.Blue, 180);
