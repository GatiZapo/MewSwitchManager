...
public sealed class UpdateService
{
    ...
    public async Task<UpdateInfo> CheckAsync(CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            {
                return await ParseReleaseAsync(releaseResponse, current, ct);
            }
            ...
            if (releaseResponse.StatusCode != HttpStatusCode.NotFound)
            {
                ...
                {
                    var remaining = releaseResponse.Headers.TryGetValues("X-RateLimit-Remaining", out var values)
        ...
        catch (HttpRequestException ex)
        {
            ...
            return UpdateInfo.Error(current, "Unable to reach GitHub. Check your internet connection.");
        }
        catch (TaskCanceledException)
        {
            _logger.Warn("GitHub update check timed out.");
            return UpdateInfo.Error(current, "GitHub update check timed out.");
        }
        catch (JsonException ex)
        {
            _logger.Warn($"Update check failed: {ex.Message}");
            return UpdateInfo.Error(current, "GitHub returned invalid update data.");
        }
        catch (Exception ex)
        {
    ...
    private async Task<UpdateInfo> ParseReleaseAsync(HttpResponseMessage response, string current, CancellationToken ct)
    {
        ...
        _logger.Info(available
            ? $"GitHub release update available: {current} -> {latest}."
            : $"GitHub release check complete: {current} is current against {latest}.");
    ...
    private async Task<UpdateInfo> CheckDevelopmentBuildAsync(string current, string? currentCommit, CancellationToken ct)
    {
        ...
        if (string.IsNullOrWhiteSpace(sha))
            return UpdateInfo.Error(current, "GitHub returned no main commit SHA.");
        ...
        if (string.IsNullOrWhiteSpace(currentCommit))
        {
            ...
            return new UpdateInfo(false, current, current, "", url, "Development build", message,
                null, null, "This installation was built without Git commit metadata. Install a current CI build to enable development update detection.",
                sha, message, url, true);
        }
        ...
        _logger.Info(available
            ...
            : $"Development CI check complete: build {shortSha} is current.");

        return new UpdateInfo(
            available,
            current,
            available ? $"{current} (CI {shortSha})" : current,
            "",
            url,
            "Latest successful main CI build",
            message,
            assetUrl,
            assetName,
            available && string.IsNullOrWhiteSpace(assetUrl) ? "A newer main build exists, but its CI artifact is not currently available." : null,
            sha,
            message,
            url,
            true);
    ...
    private async Task<(string? Url, string? Name)> FindLatestCiArtifactAsync(string targetSha, CancellationToken ct)
    {
        ...
        var runs = runsDoc.RootElement.TryGetProperty("workflow_runs", out var workflowRuns)
            ? workflowRuns : default;
...
";
    ...
    private static (string? Url, string? Name) FindReleaseAsset(JsonElement root)
    {
        ...
        foreach (var asset in assets.EnumerateArray())
        {
            ...
            if (string.IsNullOrWhiteSpace(name) || string.IsNullOrWhiteSpace(url) || !name.EndsWith(".zip", StringComparison.OrdinalIgnoreCase))
                continue;
